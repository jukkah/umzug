...
export default class Umzug extends EventEmitter {
   ...
   * })
   *
   * @param {string} [type='json'] Type of built-in storage. If omitted, it will
   *     try to guess type based on given options. Use '' (empty string) for
   *     {@link Storage}, 'json' for {@link JSONStorage}, and 'sequelize' for
   *     {@link SequelizeStorage}.
   ...
   * let storage = ...
   *
   * // construct with minimal set of parameters (don't copy-paste next line as
   * // there's a zero width space in the glob.)
   ...
   * 2. If options is a list of migrations, all pending migrations in that list
   *    will be executed in a given order.
   ...
   *     execute (exclusive).
   * @param {string|Migration} [migrations.to=∞] The last migration to execute
   *     (inclusive).
   * @param {object} [options]
   ...
   *     _executed_ (true) or _skipped_ (false) and Error if there was an error.
   ...
   * 2. If options is a list of migrations, all executed migrations in that list
   *    will be reverted in a given order.
   ...
   *     execute (exclusive).
   * @param {string|Migration} [migrations.to=0] The last migration to execute
   *     (inclusive).
   * @param {object} [options]
   * @param {boolean} [options.failFast=true] Fail, when first migration fails.
   * @returns {Promise<Map<Migration, boolean>, Map<Migration, boolean|Error>>}
